...
def storeWithRetries(record: Record, retries: Int): Future[Either[StorageError, DateTime]] = {
  service.store(record).recoverWith {
    case DuplicateId(_) =>
      if (retries > 0)
        storeWithRetries(record.cloneWithNewId(idGenerator.next()))
      else
        throw new RetryLimitExceededException("All retries used up, I have no idea what to do next")
  }
}
```
...
object Service {
  ...
  final case class DuplicateRecordId(record: Record) extends StorageError("Duplicate ID in record " + record)
